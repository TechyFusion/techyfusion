    ...
    <div className="relative min-h-screen pb-20">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-24">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
          <div className="card-soft rounded-2xl p-8 space-y-4">
            <div className="w-12 h-12 rounded-xl bg-accent-blue/10 border border-accent-blue/25 flex items-center justify-center text-accent-blue mx-auto">
              <Sparkles className="w-6 h-6" />
            </div>
            <h3 className="text-lg font-[family-name:var(--font-inter)] font-semibold text-charcoal">Continuous Innovation</h3>
          ...
          <div className="card-soft rounded-2xl p-8 space-y-4">
            ...
            <h3 className="text-lg font-[family-name:var(--font-inter)] font-semibold text-charcoal">Flexibility & Autonomy</h3>
            <p className="text-text-muted text-sm leading-relaxed">
          ...
          <div className="card-soft rounded-2xl p-8 space-y-4">
            ...
            <h3 className="text-lg font-[family-name:var(--font-inter)] font-semibold text-charcoal">Professional Growth</h3>
            <p className="text-text-muted text-sm leading-relaxed">
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-12">
          <h2 className="text-2xl font-[family-name:var(--font-inter)] font-bold text-charcoal">Active Opportunities</h2>
          
          {/* Category Tabs */}
          <div className="flex gap-1 bg-white/70 p-1.5 border border-border-soft rounded-xl backdrop-blur-md">
            {categories.map(cat => (
              <button
                key={cat.id}
        ...
        <div className="space-y-6">
          {filteredJobs.length === 0 ? (
            <div className="text-center py-16 bg-white/40 border border-border-soft rounded-3xl p-10">
              <p className="text-text-muted text-sm">No openings in this department at this time. Check back later!</p>
            </div>
          ) : (
            filteredJobs.map((job) => (
              <div
                key={job.id}
                className="card-soft rounded-2xl p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 group hover:border-accent-blue/25 transition-all duration-300"
              >
                <div className="space-y-3 max-w-2xl">
                  ...
                  <div className="flex flex-wrap gap-4 text-xs text-text-muted font-semibold">
                ...
                <div>
                  <button
                    ...
                    className="px-6 py-3 rounded-xl bg-white border border-border-soft text-charcoal font-semibold text-sm hover:bg-black/5 hover:text-accent-blue transition-all duration-300 flex-shrink-0"
                  >
                    View Details & Apply
                  </button>
                </div>
      ...
      {selectedJob && (
        <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-center justify-center p-4">
          <div className="card-soft rounded-3xl w-full max-w-2xl overflow-hidden relative border border-black/10 max-h-[90vh] flex flex-col justify-between">
            {/* Header */}
            <div className="p-6.5 border-b border-border-soft flex items-center justify-between">
              <div>
                <h3 className="text-xl font-[family-name:var(--font-inter)] font-semibold text-charcoal">{selectedJob.title}</h3>
                <p className="text-text-muted text-xs mt-0.5">{selectedJob.location} • {selectedJob.type} • {selectedJob.salary}</p>
              </div>
              <button
                onClick={() => setSelectedJob(null)}
                className="text-text-muted hover:text-charcoal p-2 rounded-lg hover:bg-black/5 transition-colors"
            ...
            <div className="p-6.5 overflow-y-auto space-y-6 flex-grow">
              {isApplied ? (
                <div className="py-12 flex flex-col items-center justify-center text-center space-y-4">
                  ...
                  <h4 className="text-xl font-bold text-charcoal">Application Logged!</h4>
                ...
                <div className="space-y-6">
                  {/* Job details */}
                  <div className="space-y-3 bg-black/5 p-5 rounded-2xl border border-border-soft">
                    <h4 className="text-xs font-bold text-text-muted uppercase tracking-widest">Key Requirements:</h4>
                  ...
                  <form onSubmit={handleApplySubmit} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-1">
                        <label htmlFor="app-name" className="text-[10px] text-text-muted font-semibold uppercase tracking-wider">Your Name</label>
                        <input
                          type="text"
                          id="app-name"
                          required
                      ...
                      <div className="space-y-1">
                        <label htmlFor="app-email" className="text-[10px] text-text-muted font-semibold uppercase tracking-wider">Email Address</label>
                        <input
                          type="email"
                          id="app-email"
                          required
                    ...
                    <div className="space-y-1">
                      <label htmlFor="app-resume" className="text-[10px] text-text-muted font-semibold uppercase tracking-wider">Resume PDF URL / Portfolio Link</label>
                      <input
                        type="url"
                        id="app-resume"
                        required
                    ...
                    <div className="space-y-1">
                      <label htmlFor="app-cover" className="text-[10px] text-text-muted font-semibold uppercase tracking-wider">Cover Letter / Comments (Optional)</label>
                      <textarea
                        id="app-cover"
                        rows={4}
                    ...
                    <button
                      type="submit"
                      className="w-full py-3 rounded-full bg-accent-blue hover:bg-accent-blue-hover text-white font-bold text-sm tracking-wide hover:shadow-[0_0_15px_rgba(6,182,212,0.3)] transition-all flex items-center justify-center gap-2"
                    >
